...
//arrar que almacena los enemigos
var enemigos = [];
...
function loadMedia() {
    fondo = new Image();
    fondo.src = 'space.jpg';
    fondo.onload = function () { //se ejecuta cuando la imagen se cargue
        var intervalo = window.setInterval(frameLoop, 1000 / 55);//operacion matematica para calcular el tiempo en el que se va a cargar cada uno de los frame
    };
}
...
function dibujarEnemigos() {
    for (var i in enemigos){
        ...
        if (enemigo.estado == 'muerto') ctx.fillStyle = 'blank';
...
function agregarEventosTeclado() {
    ...
    agregarEvento(document, "keyup", function (e) {
                    //ponemos en false la tecla qeu dejo de ser presionada
                   teclado[e.keyCode] = false;//evento que identifica la tecla que se ha pulsado
                   });
}
...
function moverNave() {
    ...
    if(nave.estado == 'hit'){
        ...
        if(nave.contador >= 20){
            ...
            textoRespuesta.subtitulo = 'Presiona R para volver a intentarlo';
    ...
    for(var i in enemigos){
            ...
            if(enemigo && enemigo.estado == 'vivo'){
                ...
                if(aleatorio(0,enemigos.length * 10) == 4){
                    disparosEnemigos.push(agregarDisparosEnemigos(enemigos));
                }
            }
        if(enemigos && enemigo.estado == 'hit'){
            enemigo.contador++;
...
function actualizarEstadoJuego(){
    if(juego.estado == 'jugando' && enemigos.length == 0){
        ...
        textoRespuesta.titulo = 'Derrotastes a los marcianos';
